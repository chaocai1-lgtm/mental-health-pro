...
if view_mode == "👩‍🏫 教师/管理模式" and st.session_state['is_admin_logged_in']:
    ...
    with c1:
        ...
        if trend: st_pyecharts(build_line_chart(trend), height="350px")
        else: st.info("当前周期内暂无趋势数据")
    with c2:
        st.subheader("⚠️ 风险分布")
        if risk_dist: st_pyecharts(build_pie_chart(risk_dist), height="350px")
        else: st.info("暂无数据")

    c3, c4 = st.columns(2)
    with c3:
        st.subheader("🔥 痛点词云")
        if top_pains: st_pyecharts(build_wordcloud(top_pains), height="400px")
    with c4:
        st.subheader("🏆 工具点赞榜")
        if top_methods:
    ...
    with col_ex2:
        ...
        if trend:
            st.download_button("📥 下载趋势数据 (.csv)", pd.DataFrame(trend).to_csv(index=False).encode('utf-8-sig'), "trend.csv", "text/csv")
        if top_pains:
            st.download_button("📥 下载热词数据 (.csv)", pd.DataFrame(top_pains).to_csv(index=False).encode('utf-8-sig'), "hot_words.csv", "text/csv")
